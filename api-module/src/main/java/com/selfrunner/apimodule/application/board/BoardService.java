package com.selfrunner.apimodule.application.board;

import com.google.firebase.messaging.Message;
import com.google.firebase.messaging.MulticastMessage;
import com.selfrunner.apimodule.global.auth.Auth;
import com.selfrunner.commonmodule.dto.board.request.PostBoardReq;
import com.selfrunner.commonmodule.dto.board.request.PutBoardReq;
import com.selfrunner.commonmodule.dto.board.request.ReplyReq;
import com.selfrunner.commonmodule.dto.board.response.*;
...
import com.selfrunner.domainmodule.domain.board.Board;
...
import com.selfrunner.domainmodule.domain.board.repository.BoardRepository;
import com.selfrunner.domainmodule.domain.board.repository.FileJdbcRepository;
import com.selfrunner.domainmodule.domain.board.repository.FileRepository;
import com.selfrunner.domainmodule.domain.board.repository.ReplyRepository;
...
public class BoardService {
    ...
    public BoardRes registerBoard(Member member, List<MultipartFile> multipartFileList, PostBoardReq postBoardReq) {
        ...
        MemberAndLecture memberAndLecture =
                memberAndLectureRepository.findMemberAndLectureByMemberAndLectureLectureId(member, postBoardReq.getLectureId())
                        .orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));
        // 파일 개수 최대 5개 제한 로직 && 클래스당 용량 검사 로직 추가 필요
        if(multipartFileList != null
                && checkFileCapacity(memberAndLecture.getLecture().getLectureId(), multipartFileList, null)) {
            throw new ApplicationException(ErrorCode.TOTAL_SIZE_EXCEED);
        }
        if(multipartFileList != null && multipartFileList.size() > 5) {
            throw new ApplicationException(ErrorCode.TOTAL_FILE_EXCEED);
        }
        ...
        Board board = Board.builder()
                ...
                .member(member)
                .isPublic(postBoardReq.getIsPublic())
                .lessonId(postBoardReq.getLessonId())
                .title(postBoardReq.getTitle())
                .body(postBoardReq.getBody())
                .category(postBoardReq.getCategory())
                .status(postBoardReq.getStatus())
                .build();
        Board saveBoard = boardRepository.save(board);
        LocalDate lessonDate = (board.getLessonId() != null)
                ? lessonRepository.findLessonDateByLessonId(board.getLessonId()).orElse(null)
                : null;
        // File이 존재하면 S3 업로드 진행
        List<FileRes> fileUrlList = (multipartFileList != null)
                ? uploadFileList(multipartFileList, member.getMemberId(), memberAndLecture.getLecture().getLectureId(), saveBoard.getBoardId(), null)
                : null;
        // FCM 알림 전송
        sendBoardNotification(member, memberAndLecture, board);

        // Response
        return new BoardRes(saveBoard.getBoardId(),
                saveBoard.getLecture().getLectureId(),
                member.getMemberId(),
                member.getType(),
                member.getName(),
                saveBoard.getIsPublic(),
                saveBoard.getLessonId(),
                lessonDate,
                saveBoard.getTitle(),
                saveBoard.getBody(),
    ...
    public BoardReplyRes updateBoard(Member member, Long boardId, List<MultipartFile> multipartFileList, PutBoardReq putBoardReq) {
        ...
        if(multipartFileList != null
                && checkFileCapacity(board.getLecture().getLectureId(), multipartFileList, putBoardReq.getDeleteFileList())) {
            throw new ApplicationException(ErrorCode.TOTAL_SIZE_EXCEED);
        }
        if(multipartFileList != null
                && (multipartFileList.size() > 5 || (multipartFileList.size() - putBoardReq.getDeleteFileList().size() + fileRepository.findCountByBoardId(boardId) > 5))) {
            throw new ApplicationException(ErrorCode.TOTAL_FILE_EXCEED);
        }
        ...
        // Response
        return new BoardReplyRes(
                updateBoard.getBoardId(),
                updateBoard.getLecture().getLectureId(),
                member.getMemberId(),
                member.getType(),
                member.getName(),
                updateBoard.getIsPublic(),
                updateBoard.getLessonId(),
                updateBoard.getTitle(),
                updateBoard.getBody(),
                updateBoard.getCategory(),
                updateBoard.getStatus(),
                lessonDate,
                replyCount,
                fileResList,
                updateBoard.getCreatedAt(),
                updateBoard.getModifiedAt()
        );
    }
    ...
    public void deleteBoard(Member member, Long boardId) {
        ...
        Board board = boardRepository.findById(boardId)
                .orElseThrow(() -> new ApplicationException(ErrorCode.NOT_FOUND_EXCEPTION));
        MemberAndLecture memberAndLecture = memberAndLectureRepository.findMemberAndLectureByMemberAndLectureLectureId(member, board.getLecture().getLectureId())
                .orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));
        if(!board.getMember().getMemberId().equals(member.getMemberId())
                && !memberAndLecture.getIsTeacher()) {
            throw new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION);
        }
    ...
    public BoardReplyRes changeQuestionStatus(Member member, Long boardId) {
        // Validation
        Board board = boardRepository.findById(boardId).orElseThrow(() -> new ApplicationException(ErrorCode.NOT_FOUND_EXCEPTION));
        // 질문 형태의 게시글이 아닌 경우
        if(board.getStatus().equals(QuestionStatus.GENERAL)) {
            throw new ApplicationException(ErrorCode.NOT_QUESTION_STATUS);
        }
        // 작성자와 선생님이 아니라면 상태 수정 불가능하도록 제약 조건 삽입
        MemberAndLecture memberAndLecture = memberAndLectureRepository.findMemberAndLectureByMemberAndLectureLectureId(member, board.getLecture().getLectureId())
                .orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));
        if(!board.getMember().getMemberId().equals(member.getMemberId())
                && memberAndLecture.getIsTeacher().equals(Boolean.FALSE)) {
            throw new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION);
        }
    ...
    public BoardReplyRes getOneBoard(@Auth Member member, Long boardId) {
        // Validation
        Board board = boardRepository.findById(boardId).orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));
        // 게시글 논리적 삭제 체킹
        if(board.getDeletedAt() != null) {
            throw new ApplicationException(ErrorCode.NOT_FOUND_EXCEPTION);
        }
        // 게시글 공개 여부와 권한 여부 확인
        MemberAndLecture memberAndLecture = memberAndLectureRepository
                .findMemberAndLectureByMemberAndLectureLectureId(member, board.getLecture().getLectureId())
                .orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));
        if(board.getIsPublic().equals(Boolean.FALSE)
                && !board.getMember().getMemberId().equals(member.getMemberId()) && memberAndLecture.getIsTeacher().equals(Boolean.FALSE)) {
            throw new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION);
        }
        ...
        return new BoardReplyRes(
                board.getBoardId(),
                board.getLecture().getLectureId(),
                board.getMember().getMemberId(),
                board.getMember().getType(),
    ...
    public Slice<BoardMetaRes> getBoardPagination(Member member, Long lectureId, String category, Long cursor, Pageable pageable) {
        ...
        LocalDateTime cursorCreatedAt = (cursor != null)
                ? boardRepository.findById(cursor).map(BaseTimeEntity::getCreatedAt).orElse(null)
                : null;

        // Response
        return boardRepository.findBoardPaginationByCategory(member, lectureId,
                (!category.equals("ALL")) ? BoardCategory.valueOf(category) : null,
                cursor, cursorCreatedAt, pageable);
    }
    ...
    public ReplyRes registerReply(Member member, Long boardId, List<MultipartFile> multipartFileList, ReplyReq replyReq) {
        ...
        return new ReplyRes(
                ...
                saveReply.getMember().getMemberId(),
                saveReply.getMember().getType(),
                saveReply.getMember().getName(),
                saveReply.getBody(),
                fileUrlList,
                saveReply.getCreatedAt(),
                saveReply.getModifiedAt()
        );
    }
    ...
    public void deleteReply(Member member, Long boardId, Long replyId) {
        // Validation
        Reply reply = replyRepository.findById(replyId).orElseThrow(() -> new ApplicationException(ErrorCode.NOT_FOUND_EXCEPTION));
        MemberAndLecture memberAndLecture = memberAndLectureRepository
                .findMemberAndLectureByMemberAndLectureLectureId(member, reply.getBoard().getLecture().getLectureId())
                .orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));
        // 댓글 작성자가 아니거나 다른 게시글 ID로 요청했는지 확인하는 조건문
        if((!reply.getMember().getMemberId().equals(member.getMemberId())
                && !memberAndLecture.getIsTeacher()) || !reply.getBoard().getBoardId().equals(boardId)) {
            throw new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION);
        }
    ...
    public Slice<ReplyRes> getReplyPagination(Member member, Long boardId, Long cursor, Pageable pageable) {
        ...
        boardRepository.findBoardByMemberIdAndBoardId(member.getMemberId(), boardId)
                .orElseThrow(() -> new ApplicationException(ErrorCode.UNAUTHORIZED_EXCEPTION));

        // Business Logic
        LocalDateTime cursorCreatedAt = (cursor != null)
                ? replyRepository.findById(cursor).map(BaseTimeEntity::getCreatedAt).orElse(null)
                : null;
        ...
        if(!memberList.isEmpty()) {
            ...
            if (!tokenList.isEmpty()) {
                MulticastMessage multicastMessage = fcmClient.makeMulticastMessage(tokenList,
                        studentNotification.getTitle(),
                        studentNotification.getBody(),
                        studentNotification.getName(),
                        studentNotification.getLectureId(),
                        studentNotification.getLessonId(),
                        studentNotification.getDate(),
                        studentNotification.getUrl(),
                        studentNotification.getBoardId());
